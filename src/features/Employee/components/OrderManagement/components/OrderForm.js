...
export default function OrderForm(props) {
  const { updateOrder, recordForEdit, nameButton, statusId, shippersInOrder } = props;
  ...
  const isDisable = Number(tempStatusId.current) === 2;
  ...
  return (
    <Form onSubmit={handleSubmit}>
      <Grid container>
        <Grid item xs={6}>
          <Controls.Input label="Customer" name="customer" value={values.customerName} disabled />

          <Controls.Input label="Address" name="address" value={values.address} disabled />
        ...
        <Grid item xs={6}>
          <Controls.RadioGroup
            ...
            indexs={[+tempStatusId.current - 1, +tempStatusId.current, +tempStatusId.current + 1]}
          />
          <Controls.Input
            label="Code*"
            name="code"
          ...
          <ComboBox
            ...
            disabled={+tempStatusId.current === 1 && +values.statusId === 2 ? false : true}
          />

          <div>
            <Controls.Button type="submit" text={nameButton} disabled={isDisable ? true : null} />
          </div>
        </Grid>
      </Grid>
    </Form>
